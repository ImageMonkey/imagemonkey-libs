...
class TensorflowTrainer(object):
	...
	def _run_command(self, command, cwd=None, env=None):
		process = None
		if cwd is None:
			process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=sys.stdout, shell=False, universal_newlines=True)
		else:
			if env is None:
				process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=sys.stdout, shell=False, universal_newlines=True, cwd=cwd)
			else:
				process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=sys.stdout, shell=False, universal_newlines=True, cwd=cwd, env=env)
